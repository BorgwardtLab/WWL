...
def main():
    ...
    parser.add_argument('-s', '--seed', type=int, help='Random seed for the partition', required=True)
    parser.add_argument('--crossvalidation', default=False, action='store_true', help='Enable a 5-fold crossvalidation')
    ...
    output_path = os.paht.join('output', dataset)
    results_path = os.paht.join('results', dataset)
    ...
    if dataset == 'ENZYMES':
        labels_sequence = compute_wl_embeddings_continuous(data_directory, h)
    else:
        labels_sequence = compute_wl_embeddings_discrete(data_directory, h)

    # Save embeddings to output folder
    out_name = f'{dataset}_wl_{embedding_type}_embeddings_h{h}.npy'
    np.save(os.path.join(output_path, out_name), labels_sequence)
    ...
    for train_index, test_index in cv.split(kernel_matrices[0], y):
        ...
        if args.gridsearch:
            ...
            h, gamma = kernel_params[best_params['K_idx']]
            best_h.append(h)
